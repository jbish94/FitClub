...
import { Apple, ChevronLeft, Mail, Lock } from 'lucide-react';
 ...
 * Key behaviors:
 * - If user clicks "Log In" button (isLogin = true), they are treated as a returning user
 * - If user signs up but has existing data with the same email, they're treated as returning
 * - Returning users skip onboarding and go directly to home screen with saved preferences
 * - New users go through the 3-step onboarding to set userType, interests, and location
 */
export function LoginScreen({ onLogin, onBack, mode = 'signup' }: LoginScreenProps) {
  ...
  const handleGoogleAuth = () => {
    ...
    setTimeout(() => {
      ...
      const existingData = localStorage.getItem('fitclub_user_data');
      let shouldTreatAsLogin = isLogin;
      
      if (existingData) {
        try {
          const parsed = JSON.parse(existingData);
          // If email matches and they have completed onboarding, treat as login
          if (parsed.email === 'user@gmail.com' && parsed.hasCompletedOnboarding) {
            shouldTreatAsLogin = true;
          }
        } catch (error) {
  ...
  const handleAppleAuth = () => {
    if (isLoading) return;
    setIsLoading(true);
    // Simulate OAuth flow - in production, this would redirect to Apple OAuth
    setTimeout(() => {
      // Check if this email exists in localStorage
      const existingData = localStorage.getItem('fitclub_user_data');
      let shouldTreatAsLogin = isLogin;
      ...
      if (existingData) {
        try {
          const parsed = JSON.parse(existingData);
          // If email matches and they have completed onboarding, treat as login
          if (parsed.email === 'user@icloud.com' && parsed.hasCompletedOnboarding) {
            shouldTreatAsLogin = true;
          }
        } catch (error) {
          console.error('Error checking existing user:', error);
  ...
  const handleEmailAuth = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (existingData && !isLogin) {
        try {
          const parsed = JSON.parse(existingData);
          // If email matches and they have completed onboarding, treat as login even if signing up
          if (parsed.email === email && parsed.hasCompletedOnboarding) {
            shouldTreatAsLogin = true;
          }
        } catch (error) {
          console.error('Error checking existing user:', error);
          ...
          {isLogin && (
            <div className="text-right">
              <button 
                ...
                onClick={() => {
                  console.log('Opening password reset flow for:', email);
